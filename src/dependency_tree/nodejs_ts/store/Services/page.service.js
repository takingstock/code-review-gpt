...
const findAll = (criteria, projection = {}, options = null, cb) => {
  const query = PAGE.find(criteria, projection);
  if (options) {
...
const findAllByAggregation = (
  ...
  return PAGE.aggregate(query, cb);
};
const aggregation = (query, cb) => PAGE.aggregate(query, cb);
const count = (criteria, cb) => PAGE.countDocuments(criteria, cb);

const create = (objToSave, cb) => new PAGE(objToSave).save(cb);

const createMany = (arrayToSave, cb) => PAGE.insertMany(arrayToSave, { lean: true }, cb);

const update = (criteria, dataToSet, options = {}, cb) => PAGE
  .findOneAndUpdate(criteria, dataToSet, options, cb);

const updateAll = (criteria, dataToSet, options, cb) => PAGE
...
const deleteMany = (criteria, cb) => PAGE
  .deleteMany(criteria, cb);
const fetchDistinct = (field, criteria, callback) => {
  PAGE.distinct(field, criteria, callback)
}
module.exports = {
  findAll,
  findOne,
