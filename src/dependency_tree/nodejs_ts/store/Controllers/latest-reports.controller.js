...
const { documentService, roleService, customersService, userService, tenantService, reportsListService, queueLogsService, globalMappingService } = require('../Services');
            ...
            for (const document of documentsArray) {
                ...
                if (document.pageArray) {
                    for (const page of document.pageArray) {
                        ...
                        for (const content of page.nonTabularContent) {
                            ...
                            formattedDataArray[0][`Confidence Score ${content.global_key}`] = ''
                            formattedDataArray[0][`Confidence Threshold ${content.global_key}`] = ''
                            ...
                            if (content.global_key) {
                                ...
                                pageDetails[`Confidence Score ${content.global_key}`] = content['confidence_score'] || ''
                                pageDetails[`Confidence Threshold ${content.global_key}`] = content['confidence_threshold'] || ''
                            }
                        }
                ...
                if ("docType" in document && document.docType !== '#NEW_FORMAT#') {
                    ...
                    const formattedObj = {
                        ...
                        "Feedback match doc": (document.document_metadata && document.document_metadata.feedback_match_score) || '',
                        "Feedback match score": (document.document_metadata && document.document_metadata.feedback_match_doc) || '',
                        ...pageDetails,
                        ...pageDetailsFeedBack
                    }
                    formattedDataArray.push(formattedObj);
...
const queueLogsReport = ({ id, name, tenantId }, payload, callback) => {
    // const criteria = {
    //     isDeleted: false,
    ...
    async.auto({
        ...
        updateReportsListDB: ['sendEmail', (res, cb) => {
            ...
            reportsListService.update({ _id: reportId }, { $set: dataToSet }, { lean: true }, (err) => {
                ...
                console.log("QUEUE_LOG report successfully updated into reportlist")
                cb()
            })
        }]
    }, (err) => {
...
/** RAISE REPORT TICKET */
const raiseDocLifecycleReportTicket = (user, payload, callback) => {
    async.auto({
        insertIntoReportList: (cb) => {
            const dataToInsert = {
            ...
            reportsListService.create(dataToInsert, cb);
        }
    }, (err) => {
        if (err) {
            return callback(err);
...
const raiseClassificationReportTicket = (user, payload, callback) => {
    async.auto({
        insertIntoReportList: (cb) => {
            const dataToInsert = {
                createdByUser: user.email,
...
const raiseExtractionReportTicket = (user, payload, callback) => {
    async.auto({
        insertIntoReportList: (cb) => {
            const dataToInsert = {
                createdByUser: user.email,
...
const raiseMTDReportTicket = (user, payload, callback) => {
    async.auto({
        insertIntoReportList: (cb) => {
            const dataToInsert = {
                createdByUser: user.email,
...
const raiseExtractionReportBackupTicket = (user, payload, callback) => {
    async.auto({
        insertIntoReportList: (cb) => {
            const dataToInsert = {
                createdByUser: user.email,
...
const raiseQueueLogsReportTicket = (user, payload, callback) => {
    async.auto({
        insertIntoReportList: (cb) => {
            const dataToInsert = {
                createdByUser: user.email,
                reportName: `QUEUE_LOG_${payload.startDate}-${payload.endDate}`,
                status: "QUEUED",
                requestData: { user, payload },
                reportType: "QUEUE_LOG",
                isDownloaded: false,
                reportLinks: [],
                tenantId: user.tenantId
            }
...
const generateReport = (report, callback) => {
    console.log("REPORT:::::", report)
    const { _id: reportId, reportType, requestData: { payload = null, user = null } } = report;
    if (!payload || !user) {
    ...
    switch (reportType) {
        case 'DOC_LIFECYCLE':
            console.log('DOC_LIFECYCLE report started');
            getDocLifecycleReport(user, payload, callback)
            break;
        case 'EXTRACTION':
            console.log('EXTRACTION report started');
            getExtractionReport(user, payload, callback)
            break;
        case 'CLASSIFICATION':
            console.log('CLASSIFICATION report started');
            getClassificationReport(user, payload, callback)
            break;
        case 'MTD':
            console.log('MTD report started');
            getMTDReport(user, payload, callback)
            break;
        case 'EXTRACTION_FROM_BACKUP':
            console.log('EXTRACTION_FROM_BACKUP report started');
            getExtractionReportBackup(user, payload, callback)
            break;
        case 'QUEUE_LOG':
            console.log('QUEUE_LOG report started');
            queueLogsReport(user, payload, callback)
            break;
        default:
            callback(null, { skip: true })
    }
}
const readReportTickets = (callback) => {
    async.auto({
        report: (cb) => {
            const criteria = { status: "QUEUED" };
            reportsListService.findOne(criteria, cb);
        },
        generateReport: ['report', ({ report }, cb) => {
            if (!report) {
                return cb()
            }
            generateReport(report, cb)
        }],
    }, (e, { report, generateReport }) => {
        if (e || (generateReport && generateReport.skip)) {
            console.log("readReportTickets e:", e)
            console.log("readReportTickets generateReport.skip:", generateReport.skip)
            const dataToSet = {
                status: "FAILED"
            }
            reportsListService.updateOne({ _id: report._id }, { $set: dataToSet }, { lean: true }, (err) => {
                console.log("report updated on ticket check:", err)
                callback()
            })
        } else {
            callback()
...
const generateDailyReports = ({ name, tenantId, time }, callback) => {
    ...
    async.auto({
        ...
        docTypes: ["getTenantId", (_, cb) => {
            globalMappingService.findAll({ tenantId }, { documentType: 1 }, null, cb);
        }],
        generateExtractionReport: ["generateClassificationReports", "docTypes", ({ docTypes }, cb) => {
            let extractionReports = ``
            async.eachSeries(docTypes, (gm, escb) => {
                const data = {
                    ...payload,
                    docType: gm.documentType
                }
                getExtractionReport({ name: "SYSTEM", tenantId, time }, data, (err, res) => {
                    if (err) {
                        return escb(err)
                    }
                    extractionReports += `${gm.documentType}: ${res} <br>`;
                    escb(null, res)
                })
            }, (err) => {
                if (err) {
...
module.exports = {
    readReportTickets,
    raiseDocLifecycleReportTicket,
    raiseClassificationReportTicket,
    raiseExtractionReportTicket,
    raiseMTDReportTicket,
    raiseExtractionReportBackupTicket,
    raiseQueueLogsReportTicket,
